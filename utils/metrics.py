...
def binary_class_metrics(all_prob, all_pred, all_gold, id2label, save_path, curve_save_path):
    # 初始化一个空列表来保存结果
    results = []
    ...
    for i, label in id2label.items():
        ...
        # print(f"label:{label} - auc: {auc:.4f} - aupr: {aupr:.4f} - f1: {f1:.4f} - acc: {acc:.4f} - sn: {sn:.4f} - sp: {sp:.4f} - mcc: {mcc:.4f}")

        # 计算 ROC 曲线的数据
        fpr, tpr, _ = roc_curve(y_true=all_gold[:, i], y_score=all_prob[:, i])
        roc_data = pd.DataFrame({'FPR': fpr, 'TPR': tpr})
        roc_data.to_csv(f"{curve_save_path}/{label}_roc.csv", index=False)

        # 计算 PR 曲线的数据
        precision, recall, _ = precision_recall_curve(y_true=all_gold[:, i], probas_pred=all_prob[:, i])
        pr_data = pd.DataFrame({'Precision': precision, 'Recall': recall})
        pr_data.to_csv(f"{curve_save_path}/{label}_pr.csv", index=False)
    ...
    with open(save_path, 'w') as f:
        for result in results:
            ...
            f.write(f"Sensitivity (SN): {result['sn']:.4f}\n")
            f.write(f"Specificity (SP): {result['sp']:.4f}\n")
